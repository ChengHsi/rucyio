...
def add_identity(validLdapObject):
    """
    add LDAP entry as Rucio Identity
    """
    for attr in validLdapObject:
        try:
            if 'account' in retrieveAttributes:
                account = attr[1]['account'][0]
            else:
                account = attr[1]['uid'][0]
            if 'auth_type' in retrieveAttributes:
                authtype = attr[1]['auth_type'][0]
            else:
                authtype = 'x509'  # Default authtype set as X509
            identity = attr[1]['gecos'][0]
            email = attr[1]['mail'][0]
            add_account(account)
...
def validate_ldap_entries(ldapObject):
    """
    function to validate if ldap entries fit Rucio schema, currently only a quick account validation
    """
    results = ldapObject.search_s(baseDN, searchScope, searchFilter, retrieveAttributes)
    results_list = list(results)
    for result in results:
        uid = result[1]['uid'][0]
        if not re.match(r'^[a-z0-9-_]{1,30}$', uid):
            results_list.remove(result)
            print 'uid: \'' + uid + '\' is invalid as Rucio account'
    return results_list
...
if __name__ == '__main__':
    v = validate_ldap_entries(initiate_ldap())
    add_identity(v)
